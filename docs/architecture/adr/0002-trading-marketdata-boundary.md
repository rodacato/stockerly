...
4. **The Trading-side FX resolver self-documented the leak.** (It was replaced by `Trading::Domain::FxConversion` in 2026-08-29's amendment to ADR-009; the boundary argument below is unchanged.) `fx_rate_resolver.rb:20` reads: *"Cross-context call to MarketData::Gateways::FxRatesGateway is a known leak."* That comment is the symptom: the rule exists, the codebase violates it, and the violator knows it. Either the rule is wrong, or the codebase needs aggressive refactoring.
...
- **FX access** stays a tolerable shared-model dependency. The FX-storage ADR this clause waited for is [ADR-009](0009-fx-history-strategy.md), and it did not move the models out of the top level — so `FxRate` and `FxRateHistory` remain shared, read directly by `Trading::Domain::FxConversion`. See the amendment at the foot of this ADR for why the wrapper that used to sit in front of them is gone.
