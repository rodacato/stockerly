...
- [ ] **Honor the 24h pause for the S07→S08 transition.** This is the second consecutive override of the 24h rule. A third consecutive override would mean the rule is broken in practice, not occasionally excepted. If S08 opens within 24h of this close, the rule should be deleted from the protocol — not preserved as fiction. Concrete: wait until 2026-05-17 sometime to open S08 (or kill the rule).
- [ ] **Auth-flow translation to es-MX.** Residual English lives in `app/views/registrations/new.html.erb` form labels/placeholders/button, `app/views/sessions/new.html.erb` (entire login form), `shared/_public_footer` (privacy/terms/risk links + "Open Source Market Intelligence Platform" tagline + copyright bottom bar), and dry-validation default messages surfaced anywhere errors render to the user. Worth its own scoped S08 issue with a complete discovery card.
- [ ] **Continue Lumen migration for the remaining 59 hardcoded color classes.** S07 closed at 59 (down from 62), but the migration is not done. Most residue is in admin views (`/admin/users`, `/admin/dashboard`), the public footer, and some shared partials. Worth either a focused mini-sprint or rolling into the auth-flow translation issue if those overlap.
- [ ] **Regenerate screenshots in `docs/screenshots/`.** S07 added 5 new screens (`/welcome`, `/help`, `/report-bug`, `/admin/invites`, rewritten `/privacy`) and `qa.md` flagged screenshots as pending. They are not in `docs/screenshots/` at close. Visual changes were verified against mockups and through manual smoke during implementation, but the canonical screenshots step from the `_template/qa.md` was skipped. Catch up on this in S08 open as a small task before any new visual work lands.
- [ ] **Read Rails-generated stubs before overwriting them.** The migration self-destruct on #70 (`db/migrate/.../create_invite_codes.rb` was generated empty, I ran `db:migrate` before Writing the real content, schema went out of sync, had to drop the table and remove the schema_migrations row manually). Pattern to internalize: any file Rails generates with placeholder content needs Read first, then Write — even when Write is going to replace 100% of the content. The Write tool's "must Read first" guard is the right tripwire.
