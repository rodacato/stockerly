...
Declared customer/supplier pairs, each one-directional: **Trading → MarketData** (ADR-002), **Alerts → MarketData** (ADR-002's 2026-09-04 amendment), and **Alerts → Trading** ([ADR-025](docs/architecture/adr/0025-alerts-reads-trading.md)), whose read crosses as plain data — `Trading::Queries::OpenHoldings` returns hashes, not `Position` records. Further pairs adopt the pattern via additional ADRs.
...
| Expert Panel (core, situational, and the user's seat) | `docs/vision/experts.md` |
