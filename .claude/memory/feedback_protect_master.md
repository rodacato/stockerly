...
**How to apply:**
- When creating a branch for a PR, use `git fetch origin && git checkout -b BRANCH origin/master --no-track`. `fetch` ensures the new branch starts from the latest remote state (not a stale local `master`); `--no-track` is the load-bearing flag — it prevents Git from auto-setting the new branch's upstream to `origin/master`, which is exactly the misconfiguration that caused the 2026-05-16 incident.
