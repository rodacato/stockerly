...
| **`MovementItem` has no age slot** | `cockpit.pen`, 2026-09-16 — `dashboard/_signal_row` prints `hace 4 días` in mono 11 under the phrase for any reading older than today | Drawn inside the instance's `Reason` text on a second line, in its sans 12. Belongs with the structural `MovementItem` deviation already measured for the bump |
| **`MovementItem` has no slot under its tag** | `cockpit.pen`, 2026-09-17 — `dashboard/_signal_row` stacks the verb chip over `Tendencia` or `Rebote` in mono 10 (D127) | Each signal instance replaces `Tag` with a `Verbo` column holding the chip and the label. Same bump as the age slot: both are the signal row outgrowing a component shared with trade rows |
