...
RSpec.describe CreateAuditLogOnLoginFailure do
  let!(:user) { create(:user, email: "target@example.com") }

  it "creates an audit log when user exists" do
    event = UserLoginFailed.new(email: "target@example.com", ip_address: "10.0.0.1", user_agent: "curl/7.0")
  ...
  it "does not create an audit log when user does not exist" do
    event = UserLoginFailed.new(email: "nobody@example.com", ip_address: "10.0.0.1", user_agent: "curl/7.0")
