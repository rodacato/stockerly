...
RSpec.describe Administration::UseCases::Users::SuspendUser do
  ...
  describe "#call" do
    ...
    it "publishes Identity::Events::UserSuspended event" do
      handler = double("handler")
      allow(handler).to receive(:call)
      EventBus.subscribe(Identity::Events::UserSuspended, handler)

      described_class.call(user_id: regular_user.id, admin: admin)
      expect(handler).to have_received(:call).with(an_instance_of(Identity::Events::UserSuspended))
    end
  end
end
