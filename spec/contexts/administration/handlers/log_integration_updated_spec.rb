...
RSpec.describe Administration::Handlers::LogIntegrationUpdated do
  describe ".call" do
    let(:event) { Administration::Events::IntegrationUpdated.new(integration_id: 1, provider_name: "Alpaca", changes: { "daily_call_limit" => { from: 500, to: 1000 } }) }
    ...
    it "logs with success severity" do
      ...
      log = SystemLog.last
      expect(log.task_name).to eq("Integration Updated: Alpaca")
