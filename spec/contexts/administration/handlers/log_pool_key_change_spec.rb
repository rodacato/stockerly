...
RSpec.describe Administration::Handlers::LogPoolKeyChange do
  describe ".call" do
    it "logs pool key addition with success severity" do
      event = Administration::Events::PoolKeyAdded.new(integration_id: 1, pool_key_id: 1, key_name: "Production")
    ...
    it "logs pool key enable with success severity" do
      event = Administration::Events::PoolKeyToggled.new(pool_key_id: 1, key_name: "Backup", enabled: true)
      described_class.call(event)
      expect(SystemLog.last.task_name).to eq("Pool Key Enabled: Backup")
    end

    it "logs pool key disable with success severity" do
      event = Administration::Events::PoolKeyToggled.new(pool_key_id: 1, key_name: "Old Key", enabled: false)
      described_class.call(event)
      expect(SystemLog.last.task_name).to eq("Pool Key Disabled: Old Key")
    end

    it "logs pool key removal with warning severity" do
      event = Administration::Events::PoolKeyRemoved.new(pool_key_id: 1, key_name: "Retired")
      described_class.call(event)
