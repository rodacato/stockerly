...
RSpec.describe MarketData::Gateways::FmpGateway do
  ...
  describe "API key resolution" do
    context "when Integration record exists with valid key" do
      before do
        integration = create(:integration, provider_name: "FMP", api_key_encrypted: "db_key")
    ...
    context "when no Integration record exists" do
      it "raises ApiKeyNotConfiguredError" do
        expect { described_class.new }.to raise_error(
          MarketData::Gateways::ApiKeyNotConfiguredError, /FMP/
        )
      end
    end

    context "when Integration exists but api_key_encrypted is nil" do
      before { create(:integration, :keyless, provider_name: "FMP") }
