...
RSpec.describe Trading::DeleteTrade do
  ...
  describe "event publishing" do
    it "publishes TradeDeleted event" do
      handler = class_double(LogTradeDelete, call: nil).as_stubbed_const
      EventBus.subscribe(TradeDeleted, handler)

      result = described_class.call(user: user, trade_id: trade.id)

      expect(result).to be_success
      expect(handler).to have_received(:call).with(an_instance_of(TradeDeleted))
    end
  end
