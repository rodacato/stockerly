...
RSpec.describe Trading::UpdateTrade do
  ...
  describe "event publishing" do
    it "publishes TradeUpdated event" do
      handler = class_double(LogTradeUpdate, call: nil).as_stubbed_const
      EventBus.subscribe(TradeUpdated, handler)

      result = described_class.call(user: user, params: { trade_id: trade.id, shares: 15.0 })

      expect(result).to be_success
      expect(handler).to have_received(:call).with(an_instance_of(TradeUpdated))
    end
  end
