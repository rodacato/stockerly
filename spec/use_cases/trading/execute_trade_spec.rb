...
RSpec.describe Trading::ExecuteTrade do
  ...
  describe "event publishing" do
    it "publishes TradeExecuted event" do
      handler = class_double(RecalculateAvgCostOnTrade, call: nil).as_stubbed_const
      EventBus.subscribe(TradeExecuted, handler)

      result = described_class.call(user: user, params: buy_params)

      expect(result).to be_success
      expect(handler).to have_received(:call).with(an_instance_of(TradeExecuted))
    end
  end
