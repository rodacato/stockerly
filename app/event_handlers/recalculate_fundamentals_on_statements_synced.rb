class RecalculateFundamentalsOnStatementsSynced
  def self.call(event)
    ...
    metrics = FundamentalCalculator.calculate(
    ...
    ttm_income = FundamentalCalculator.calculate_ttm(quarterly_income)
    ttm_cf = FundamentalCalculator.calculate_ttm(quarterly_cf)
    ...
    EventBus.publish(AssetFundamentalsUpdated.new(
      asset_id: asset.id,
      symbol: symbol,
